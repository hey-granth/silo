...
class File(models.Model):
    ...
    owner_id = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name="files", db_index=True)
...
class FileChunk(models.Model):
    id = models.UUIDField(primary_key=True)
    file_id = models.ForeignKey(File, on_delete=models.CASCADE, related_name="chunks", db_index=True)
...
class FileAccessLog(models.Model):
    ...
    file_id = models.ForeignKey(File, on_delete=models.CASCADE, related_name="access_logs", db_index=True)
    user_id = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name="file_access_logs", db_index=True)
    ip_address = models.GenericIPAddressField(null=True, blank=True)
    user_agent = models.CharField(max_length=255, null=True, blank=True)
    action = models.enums.TextChoices('Action', 'UPLOAD DOWNLOAD DELETE SHARE')
    timestamp = models.DateTimeField(auto_now_add=True)
    token = models.CharField(max_length=255, null=True, blank=True)  # for sharing via token links
...
class SharedFileLink(models.Model):
    id = models.UUIDField(primary_key=True)
    owner = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name="shared_links", db_index=True)
    file_id = models.ForeignKey(File, on_delete=models.CASCADE, related_name="shared_links", db_index=True)
    token = models.CharField(max_length=255, unique=True, db_index=True)  # unique token for sharing
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    password_hash = models.CharField(max_length=255, null=True, blank=True)  # to make it password protected
